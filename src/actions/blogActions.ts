...
export async function handleGeneratePost(
  ...
  try {
    ...
    try {
      const heroImagePrompt = `Generate a hero image for a blog post titled: "${title}". CRITICAL: Prioritize generating an image that is highly web-optimized for a small file size (e.g., under 250KB if possible, ideally PNG format with efficient compression). The image must be visually compelling and directly relevant to the post's core subject. Avoid text in the image. Style: modern, clean, professional.`;
    ...
    // Check if the error message contains "FALLBACK_BODY_TOO_LARGE"
    if (error instanceof Error && error.message.includes('FALLBACK_BODY_TOO_LARGE')) {
        errorMessage = 'Failed to generate post: The generated post is too large, likely due to oversized images. Try reducing image complexity or number, or simplify the post title for less complex image prompts.';
    ...
    return { message: errorMessage, success: false, postStatus: 'error', fields: rawFormData };
  }
}
